...
import {download, getCollectUserId, setCollect} from '../api/interface'
...
export default {
  ...
  methods: {
    ...
    // 显示播放中的歌曲列表
    changeAside() {
      this.$store.state.showAside = !this.$store.state.showAside
      this.$store.commit('setShowAside', this.$store.state.showAside)
    },
    // 上一首
    prev() {
      if (this.listIndex !== -1 && this.listOfSongs.length > 1) { // 当前处于不可能状态或只有一首音乐时不执行
    ...
    toplay(url) {
      if (url && url !== this.url) {
        ...
        this.$store.commit('setPicUrl', this.$store.state.configure.HOST + this.listOfSongs[this.listIndex].picture)
    ...
    parseLyric(text) {
      ...
      // 遍历每一行,形成带着两个元素的数组.前面是时间,后面是歌词
      for (let item of lines) {
        let time = item.match(pattern) // 存前面的时间
